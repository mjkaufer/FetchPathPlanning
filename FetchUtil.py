...
def axisToRoll(axis='x'):
    ...
    raise ValueError("Invalid axis! You passed in '" + str(axis) + "'' but valid axes are x, y, or z")

def xRotationMatrix(rotationInRadians):
    r = rotationInRadians
    return np.matrix([
        [1, 0, 0],
        [0, np.cos(r), -np.sin(r)],
...
def buildRotationMatrix(rotationAxis, rotationInRadians):
    xRotation = xRotationMatrix(rotationInRadians)
    rollAmount = axisToRoll(rotationAxis)
